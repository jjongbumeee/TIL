...
- 가비지 컬렉터
    - memory leak : 프로그래머가 비워줘야할 메모리 공간을 비우지 않아 생기는 문제
        - *Mark & Sweep* 방식 : 포인터를 따라가 루트에사 연결되지 않은 부분 메모리 해제
        - *Reference counting* : 한 요소가 다른 요소에게 얼마나 참조되었는지 세는 방법(0되면 해제)
        - garbarge collector는 완벽하지 않음 (프로그래머의 관리가 필요)
    - managed language : 가비지 컬렉터가 메모리 공간을 자동으로 관리해주는 언어(Java 등)
    - unmanage language : 프로그래머가 메모리 관리를 수동적으로 해줘야하는 언어
    - **순환 참조하지 말 것**
- 예외처리
    - `try - catch - finally`에서 `finally`는 함수가 리턴될 경우에도 실행 후 반환
- 변수의 스코프 (const, let, var)
    - `var`는 ES6 이전의 JS에서 사용되는 변수 선언 문법, 재선언 가능
    - `const`는 상수로 한번 선언되면 값 변경 불가
    - `let`는 `var`를 대체하지만 재선언 불가
    - `const`와 `let`은 **블록 레벨 스코프**를 가짐  
        - 따라서 블록 외부로 접근 불가
        - 블록 종료 시 변수 할당 해제
        - 선언되기 이전 사용(hoisting) 불가
    - 반면에 `var`는 **함수 스코프**를 가지고 있음  
        - 따라서 함수 내에서는 어디든 접근이 가능
        - 변수가 해제되지 않음
        - hoisting을 통해 선언되기 이전에도 값 사용 가능
- 재귀함수
    - 재귀함수는 계속 이어질 경우 스택 오버플로와 성능 저하의 문제가 발생
    - `return`시에 별도의 계산이 필요하지 않은 재귀함수는 **꼬리 재귀 최적화(Tail Recursion Optimization)**로 선형시간 계산 가능
- 비동기 프로그래밍
    - 동기적으로 작동한다는 것은 *코드가 반드시 작성된 순서대로 실행됨을 의미*
    - 반면 비동기적으로 작동한다는 것은 *코드가 순서대로 실행되지 않음을 의미*
    - 동기적으로 작동하기 위해서는 스레드, 프로세스가 2개 이상이 동시에 실행되어야 함
    ```js
    function asyncBlackBeanTimer(seconds) {
        ...
        setTimeout(
            ...
            seconds*1000; //해당 코드 실행 후
        );
        console.log("배달부 떠남");
    }
    asyncBlackBeanTimer(1);
    ```
    - 비동기 작업이 끝난 이후에는 Call-back function을 호출해 이후 업무 처리
    - 자바스크립트 엔진은 싱글 스레드로 동작, 하지만 JS 엔진과 Web API가 동시에 동작
    - Web API는 JS엔진과는 달리 동시에 여러작업을 처리 가능(멀티스레드)
    - 따라서 비동기 작업의 경우 Web API에서 처리 후 Task-Queue로 콜백함수(싱글 스레드 처리 가능하도록) 보냄
    - 비동기 작업을 Web API에서 처리하는 동안 JS엔진은 동기 작업을 수행
    - 이벤트 루프는 Task-queue에 도착한 순서대로 콜백함수들을 다시 JS 처리 엔진으로 보냄
